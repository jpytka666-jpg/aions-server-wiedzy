...
|-------|------|--------------|---------------|
| **`aions-context`** | **Prod** (codzienna praca) | `E:\server wiedzy\venv\Scripts\python.exe` → `__main__.py stdio` | `E:\server wiedzy\data\chroma`, `D:\AIONS-INTEGRATION\aions_core` |
| **`aions-dev`** | **Dev** (WSL staging na D:) | `D:\AIONS_DEV\start_aions_dev.bat mcp` → WSL `start_aions_dev.sh mcp` | `D:\AIONS_DEV\data\chroma`, ten sam `AIONS_PATH` (WSL) |
