...
def fast_search(query: str, max_results: int = 50, folder: str = "") -> str:
    try:
        ...
        files = payload["files"]
        return _success({"query": search_query, "folder": folder or None, "files": files[:max_results], "count": len(files)})
    except Exception as e:
        return _error(str(e))
...
def system_health() -> str:
    try:
        ...
        try:
            from desktop_control import is_desktop_ready, desktop_unavailable_msg
            health["desktop"] = "ok" if is_desktop_ready() else desktop_unavailable_msg()
...
except ImportError as _desktop_import_err:
    _desktop_import_ok = False
    log(f"Desktop control module import failed: {_desktop_import_err}")


def _desktop_err(e: Exception) -> str:
    msg = str(e)
    if not _desktop_import_ok or "Desktop" in msg or "dependency" in msg.lower():
        try:
            return desktop_unavailable_msg()
        except Exception:
            pass
    return msg
...
def desktop_snapshot(target: str = "screen") -> str:
    try:
        payload = run_desktop(lambda: impl_snapshot(target=target), timeout=30)
...
def desktop_click(x: int, y: int, button: str = "left", double: bool = False) -> str:
    try:
        return _success(run_desktop(lambda: impl_click(x, y, button, double), timeout=15))
    except Exception as e:
        return _error(_desktop_err(e))
...
def desktop_type(text: str, clear_first: bool = False, x: int = 0, y: int = 0, click_first: bool = False, press_enter: bool = False) -> str:
    try:
        ...
        return _success(run_desktop(lambda: impl_type(text, clear_first, pos_x, pos_y, press_enter), timeout=30))
    except Exception as e:
        return _error(_desktop_err(e))
...
def desktop_key(combo: str) -> str:
    try:
        return _success(run_desktop(lambda: impl_key(combo), timeout=15))
    except Exception as e:
        return _error(_desktop_err(e))
...
def desktop_windows() -> str:
    try:
        return _success(run_desktop(impl_windows, timeout=15))
    except Exception as e:
        return _error(_desktop_err(e))
...
def desktop_focus(title: str = "", hwnd: int = 0) -> str:
    try:
        ...
        return _success(run_desktop(lambda: impl_focus(title, h), timeout=15))
    except Exception as e:
        return _error(_desktop_err(e))
...
def desktop_launch(path_or_name: str, args: str = "") -> str:
    try:
        return _success(run_desktop(lambda: impl_launch(path_or_name, args), timeout=20))
    except Exception as e:
        return _error(_desktop_err(e))
...
def desktop_scroll(x: int, y: int, amount: int = 3, direction: str = "vertical") -> str:
    try:
        return _success(run_desktop(lambda: impl_scroll(x, y, amount, direction), timeout=15))
    except Exception as e:
        return _error(_desktop_err(e))


@mcp_server.tool(name="desktop_shell", description="Run PowerShell command (max 30s timeout); returns stdout/stderr.")
@auto_logged
def desktop_shell(command: str, timeout: int = 30) -> str:
    try:
        return _success(run_desktop(lambda: impl_shell(command, timeout), timeout=35))
    except Exception as e:
        return _error(_desktop_err(e))
...
def desktop_ui_tree(max_depth: int = 4, max_nodes: int = 80) -> str:
    try:
        return _success(run_desktop(lambda: impl_ui_tree(max_depth, max_nodes), timeout=30))
    except Exception as e:
        return _error(_desktop_err(e))
